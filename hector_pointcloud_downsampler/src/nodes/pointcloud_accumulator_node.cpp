    ...
    : Node( "hector_pointcloud_accumulator_node", options )
{
  ...
  declare_parameter<double>( "new_points_publish_rate", 1.0 );
  ...
  double new_points_rate = get_parameter_or( "new_points_publish_rate", 1.0 );
  ...
  if ( aggregation_mode == "average" ) {
    hector_pointcloud_accumulator_ = std::make_shared<PointcloudAccumulator<AggregationMode::AVERAGE>>(
        *this, resolution, frame, rclcpp::Rate( rate ), rclcpp::Rate( new_points_rate ), topics );
