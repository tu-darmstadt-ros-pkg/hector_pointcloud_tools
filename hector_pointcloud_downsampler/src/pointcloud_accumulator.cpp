...
PointcloudAccumulatorBase::PointcloudAccumulatorBase( rclcpp::Node &node, double resolution,
                                                      ...
                                                      const rclcpp::Rate &accumulated_publish_rate,
                                                      const rclcpp::Rate &new_points_publish_rate,
    ...
    : frame_( frame ), resolution_( resolution ), tfBuffer( node.get_clock() ), tfListener( tfBuffer )
{
  ...
  accumulated_publish_timer_ = node.create_wall_timer(
      accumulated_publish_rate.period(), [this]() { publishAccumulatedPointcloud(); } );
  ...
  new_points_publisher_ = node.create_publisher<sensor_msgs::msg::PointCloud2>( "new_points", 1 );
  new_points_publish_timer_ = node.create_wall_timer( new_points_publish_rate.period(),
                                                      [this]() { publishNewPointsPointcloud(); } );

  new_points_cloud_.header.frame_id = frame_;
  new_points_cloud_.height = 1;
  new_points_cloud_.fields.resize( 3 );
  new_points_cloud_.fields[0].name = "x";
  new_points_cloud_.fields[0].offset = 0;
  new_points_cloud_.fields[0].datatype = sensor_msgs::msg::PointField::FLOAT32;
  new_points_cloud_.fields[0].count = 1;
  new_points_cloud_.fields[1].name = "y";
  new_points_cloud_.fields[1].offset = 4;
  new_points_cloud_.fields[1].datatype = sensor_msgs::msg::PointField::FLOAT32;
  new_points_cloud_.fields[1].count = 1;
  new_points_cloud_.fields[2].name = "z";
  new_points_cloud_.fields[2].offset = 8;
  new_points_cloud_.fields[2].datatype = sensor_msgs::msg::PointField::FLOAT32;
  new_points_cloud_.fields[2].count = 1;
  new_points_cloud_.is_bigendian = false;
  new_points_cloud_.point_step = 12;
  new_points_cloud_.is_dense = false;

  PCA_LOG_INFO(
      "PointcloudAccumulator initialized with resolution %f and frame %s. Publishing every %fs.",
      resolution_, frame_.c_str(), 1E9 / accumulated_publish_rate.period().count() );
}
...
void PointcloudAccumulatorBase::reset()
{
  ...
  new_points_cloud_.data.clear();
  new_points_cloud_.width = 0;
}

void PointcloudAccumulatorBase::publishAccumulatedPointcloud()
{
  if ( !updated_ ) {
    return;
  }
...
void PointcloudAccumulatorBase::publishNewPointsPointcloud()
{
  if ( new_points_cloud_.width == 0 ) {
    return;
  }
  new_points_cloud_.row_step = new_points_cloud_.point_step * new_points_cloud_.width;
  if ( new_points_publisher_->get_subscription_count() == 0 ) {
    new_points_publisher_->publish( new_points_cloud_ );
  }
  new_points_cloud_.data.clear();
}
...
void PointcloudAccumulatorBase::addField( const sensor_msgs::msg::PointField &field )
{
  ...
  new_points_cloud_.fields.push_back( new_field );
  new_points_cloud_.point_step += pointFieldSize( field.datatype ) * field.count;
}
    ...
    const sensor_msgs::msg::PointCloud2::SharedPtr &msg )
{
  ...
  new_points_cloud_.header.stamp = msg->header.stamp;
  updated_ = true;
}
    ...
    const Index &index, const Eigen::Vector3f &point )
{
  ...
  auto addNewPointToPointcloud = [&]( sensor_msgs::msg::PointCloud2 &cloud ) {
    cloud.width += 1;
    cloud.data.resize( cloud.width * cloud.point_step );

    auto *out = cloud.data.data();
    out += ( cloud.width - 1 ) * cloud.point_step;
    Eigen::Map<Eigen::Vector3f> point_map( reinterpret_cast<float *>( out ) );
    point_map = point;

    // Copy fields
    for ( size_t i = 3; i < cloud.fields.size(); ++i ) {
      const auto &target_field = cloud.fields[i];
  ...
  addNewPointToPointcloud( accumulated_cloud_ );
  addNewPointToPointcloud( new_points_cloud_ );
